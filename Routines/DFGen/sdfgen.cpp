...
int main(int argc, char* argv[]) {
  ...
  Vec3ui sizes = Vec3ui(round((max_box - min_box)/dx));
  ...
  std::string outname,outname_vox;
	...
	outname_vox = filename.substr(0, filename.size()-4) + std::string(".vox");
	...
	vox.grid2world(0,0) *= (max_box[0] - min_box[0]) / (vox.dims[0] - 1);
	vox.grid2world(1,1) *= (max_box[1] - min_box[1]) / (vox.dims[1] - 1);
	vox.grid2world(2,2) *= (max_box[2] - min_box[2]) / (vox.dims[2] - 1);

	vox.grid2world(0,3) = min_box[0];
	vox.grid2world(1,3) = min_box[1];
	vox.grid2world(2,3) = min_box[2];

	
	//.block(0,3,3,1) = -0.5*Eigen::Vector3f(1,1,1) - Eigen::Vector3f::Constant(dfix); // <-- padding introduces offset
